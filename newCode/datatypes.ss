...
(define-datatype expression expression?
  ...
  [oneif-exp  (condition expression?)
              (true expression?)]
  [quote-exp  (quotee (lambda (x) #t))]
  [begin-exp  (rest (list-of expression?))]
  ...
  [while-exp  (test list?)
              (args list?)]
  [case-lambda-exp (exps (list-of expression?))])


(define-datatype proc-val proc-val?
  [prim-proc    (name symbol?)]
  [closure      (var (lambda (ls) (or  (symbol? ls)  (list? ls))))
                (body list?)
                (env environment?)]
  [case-closure (closures list?)])
